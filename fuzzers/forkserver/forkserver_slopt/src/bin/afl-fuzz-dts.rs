...
use forkserver_slopt::{
    Dts, BUCKET, MAX_STACK_POW, SloptMetadata, feedback::SloptFeedback,
...
pub fn main() {
    ...
    // === SLOPT ===
    let slopt_algo = SloptMetadata::<Dts>::new(mutations.len(), MAX_STACK_POW, BUCKET.into());
    let slopt_feedback = SloptFeedback::<_, SloptMetadata<Dts>>::new(map_feedback);
    ...
    let slopt_mutator: HavocSloptScheduledMutator<_, SloptMetadata<Dts>> =
