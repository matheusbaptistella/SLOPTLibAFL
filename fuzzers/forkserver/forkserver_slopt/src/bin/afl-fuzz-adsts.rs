...
use forkserver_slopt::{
    Adsts, BUCKET, MAX_STACK_POW, SloptMetadata, feedback::SloptFeedback,
...
pub fn main() {
    ...
    // === SLOPT ===
    let slopt_algo = SloptMetadata::<Adsts>::new(mutations.len(), MAX_STACK_POW, BUCKET.into());
    let slopt_feedback = SloptFeedback::<_, SloptMetadata<Adsts>>::new(map_feedback);
    ...
    let slopt_mutator: HavocSloptScheduledMutator<_, SloptMetadata<Adsts>> =
